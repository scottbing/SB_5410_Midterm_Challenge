...
def main():
    ...
    bck = Image.open(BACKGROUND_IM + '.jpg')


    size = im.size
    print("size: ", size)
    # use PIL to resize city image to match guard
    out = im.resize(size)
    print("out.size: ", out.size)             #confirm identical size
    ...
    bg_pixels = out.load()  # create the pixel map
    ...
    # # fill in background
    #     # Apply further Tolerance to the RBG values
    #     for i in range(bck.size[0]):  # for every pixel:
    #         for j in range(bck.size[1]):
    #             px = pixels[i, j]
    #             if px == newbg:
    #                 image.putpixel((i, j))
